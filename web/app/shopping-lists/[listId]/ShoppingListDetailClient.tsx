...
export default function ShoppingListDetailClient({
  ...
  const handleAddFromPaste = (newItems: Array<{ genericName: string; quantity: number }>) => {
    newItems.forEach(({ genericName, quantity }) => {
      // paste still adds unpinned for now
    });
  };

  // Find the currently opened list item (by generic + productId from URL)
  ...
  const handleRename = () => {
    ...
    if (newName && newName.trim() !== list.name) {
      // renameList(listId, newName.trim()); // expose if you want
    }
  };

  const handleDelete = () => {
    if (confirm(`Excluir a lista "${list.name}"?`)) {
      // deleteList(listId); // expose if you want
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-white group-hover:text-emerald-400 transition-colors flex items-center gap-2">
                        {item.genericName}
                        {isPinned && (
                          <span className="text-[10px] px-2 py-0.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded cursor-pointer hover:bg-emerald-500/20"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleUnpin(item.id);
                                }}>
